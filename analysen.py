...
logging.basicConfig(level=logging.INFO, filename="/home/stoli/Python/Aktien/analysen.log", format='%(asctime)s %(message)s', datefmt='%d/%m/%Y %H:%M:%S')
...
def check_for_new_posts(feed):
    ...
    with open("/home/stoli/Python/Aktien/posts.txt") as file:
        for line in file:
            existing_posts.add(line.strip())

    # checken, ob Beiträge bereits gesendet wurden
    sent_posts = set()
    with open("/home/stoli/Python/Aktien/sent_posts.txt") as file:
        for line in file:
    ...
    with open("/home/stoli/Python/Aktien/posts.txt", "a+") as file:
    ...
    with open("/home/stoli/Python/Aktien/sent_posts.txt", "a+") as file:
        for post in new_posts:
            file.write(post.title + "\n")
